const portfolioData = [
  {
    ...
    companyUrl: "https://rememberapp.co.kr",
    companyDisplayName: "드라마앤컴퍼니",
    projects: [
      {
        title: "Task Manager — 과제 하나를 PRD부터 배포까지 소유",
        description: "구현자로서 티켓을 받는 자리에서, 과제를 정의하고 티켓을 만드는 자리로 역할을 확장. 목적조직의 개선 과제를 단일 오너로 맡아 문제 정의부터 지표 해석, PRD 작성, 실험 설계, 구현, 배포까지 전 과정을 관리. 1호 과제는 PM이 인계한 홈 추천 모듈 개편 핸드오프를 PRD로 다시 세우는 일이었고, 2호 과제는 선행 A/B 테스트가 실패하며 남긴 잔여 가설을 직접 Epic으로 작성한 건으로, 무엇을 할지 정하는 단계까지 범위에 포함",
        achievementList: [
          "한 화면에 동시 노출되는 추천 모듈 8종의 전환 지표를 UI 형태(배너 관문형 / 카드 직접 노출형)로 갈라 비교, 병목이 추천 품질이 아니라 배너를 눌러야만 공고가 보이는 관문 구조임을 같은 기간·같은 화면의 자연 실험으로 논증하고 인라인 전환안을 도출",
          "모듈 간 비교가 성립하도록 지표를 유저 단위로 재집계 — 동일 모듈이 집계 기준(유저 기준 / 유저×공고 pair 기준)에 따라 최하위권과 상위권을 동시에 받을 수 있음을 확인하고, 유리하게 부풀려지는 기준을 근거 자료에서 배제",
          "계측 가능성을 화면 설계의 제약으로 반영 — 전체보기 동선을 기존 전용 화면으로 유지해 모듈별 기여도 추적이 깨지지 않도록 결정",
          "직접 작성한 Epic에서 A/B/C 3-arm 실험을 설계 — 두 해결안이 서로 다른 가설을 겨냥하므로 하나의 To-Be로 묶지 않고 효과 귀속이 가능하도록 분리",
          "비어 있던 기대효과 항목을 주간 실측 지표에 선행 개편의 집계 델타를 적용해 정량 산출. 산출 과정에서 미완결 기간이 섞인 집계표가 델타를 왜곡하는 함정을 발견해 해당 구간을 산식에서 제외"
        ],
      ...
      },
      {
        ...
        achievementList: [
          "스택 PR 체인을 브랜치 이름 규칙이 아니라 head↔base 그래프로 복원 — 명명 규칙 밖의 PR도 누락 없이 묶이고, 무엇을 먼저 머지해야 위가 풀리는지를 매번 머릿속으로 재구성하던 작업을 제거",
          "리뷰 요청 수신 조건을 교정해 팀으로 라우팅된 요청이 통째로 누락되던 문제를 해결 — 검색 한정자를 넓히고 직접 요청/팀 요청 구분은 이미 받아 온 응답 데이터로 판정해 추가 호출 없이 필터를 구현",
          "PR 행에서 코드 리뷰와 리뷰 반영을 Claude Code로 바로 넘기는 버튼 — 저장소를 찾아 들어가 명령을 다시 입력하던 왕복을 클릭 하나로 축소. 실행 경로를 단일 함수로 고정해 기능이 늘어도 검증 범위가 늘지 않도록 설계",
          "Jira 연동에서 상태 이름을 코드에 하드코딩하지 않음 — 탭은 어느 인스턴스에나 보장되는 statusCategory로 나누고 필터 칩은 응답에 실제로 존재하는 상태로 런타임 생성해, 프로젝트마다 다른 상태 명칭에 깨지지 않는 구조 확보",
          "테스트 타겟이 없는 GUI 앱의 검증 절차를 스크린샷 증적 기반으로 규칙화하고, 반복해서 밟은 플랫폼 함정을 프로젝트 가이드 문서에 되돌리지 말 것 규칙으로 고정"
        ],
        tags: ["SwiftUI", "Swift 6", "macOS", "GraphQL", "Developer Tooling", "Productivity"]
      }
    ]
  },
  {
    ...
    projects: [
      ...
      },
      {
        ...
        description: "StatCounter 한국 데이터 분석을 통해 iOS 16 사용자 비율이 2.8%까지 감소한 시점에 최소 지원 버전 상향을 건의. 업계 동향(카카오톡 iOS 17 최소 지원), 프로젝트 내 #available(iOS 17) 분기 13곳 현황 등을 데이터 기반으로 정리하여 의사결정 근거를 제공",
        achievementList: [
          "프로젝트 전반의 #available(iOS 17) 분기 코드 13곳 제거로 코드 간결화"
        ],
        tags: ["iOS 17", "Migration", "Technical Decision"]
      },
      {
        title: "Fastlane 도입",
